...
type Props = { params: Promise<{ sid: string; aid: string; qid: string }> }

export const metadata = { title: 'Edit Question — AQA Teacher' }

export default async function EditTeacherQuestionPage({ params }: Props) {
  const { sid, aid, qid } = await params
    ...
    <div className="mx-auto max-w-2xl space-y-6 p-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          {question.assessment.subject.title} › {question.assessment.title}
        </p>
      </div>
      <QuestionForm
        assessmentId={aid}
