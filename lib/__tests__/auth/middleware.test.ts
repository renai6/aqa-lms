...
describe("middleware", () => {
  ...
  it("forwards user headers on the downstream request for valid ADMIN on /admin", async () => {
    vi.mocked(verifySessionToken).mockResolvedValue({
      sub: "u2",
      role: "ADMIN",
      email: "admin@x.com",
    ...
    // request, which Next encodes as x-middleware-request-* on the response.
    expect(res.headers.get("x-middleware-request-x-user-id")).toBe("u2");
