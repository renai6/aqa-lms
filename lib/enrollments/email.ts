...
export async function sendEnrollmentApprovalEmail(params: {
  ...
  const { error } = await resend.emails.send({
    ...
    html: `<p>Dear ${params.firstName},</p>
<p>Your enrollment to <strong>${params.courseName}</strong> has been approved!</p>
...
export async function sendEnrollmentRejectionEmail(params: {
  ...
  const { error } = await resend.emails.send({
    ...
    html: `<p>Dear ${params.firstName},</p>
<p>Thank you for your interest in <strong>${params.courseName}</strong> at Al-Qur'an Academy.</p>
