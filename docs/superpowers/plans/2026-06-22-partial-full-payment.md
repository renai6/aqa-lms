...
const enrollSchema = z.object({
  ...
  paymentType: z.enum(['PARTIAL', 'FULL'], { error: 'Please select a payment type.' }),
...
export async function approveEnrollmentAction(
  ...
  if (typeof id !== 'string' || !id) return { error: 'Invalid enrollment request ID.' }

  const paymentStatusRaw = formData.get('paymentStatus')
  if (paymentStatusRaw !== 'PARTIALLY_PAID' && paymentStatusRaw !== 'FULLY_PAID') {
  ...
  try {
    ...
    if (msg === 'ALREADY_PROCESSED') return { error: 'This request has already been processed.' }
...
export async function submitAdditionalPaymentAction(
  ...
  const note = typeof formData.get('note') === 'string' ? (formData.get('note') as string).trim() || null : null
...
export function AdditionalPaymentForm() {
  const [state, formAction, isPending] = useActionState(submitAdditionalPaymentAction, { error: null })
...
export async function sendPaymentStatusEmail(_params: {
  to: string; firstName: string; courseName: string
  paymentStatus: 'PARTIALLY_PAID' | 'FULLY_PAID'
  totalPaid: number; tuitionFee: number | null
}): Promise<void> {
...
export function UpdatePaymentStatusForm({ requestId, currentStatus }: Props) {
  const [state, formAction, isPending] = useActionState(updatePaymentStatusAction, { error: null })
...
export async function sendPaymentStatusEmail(params: {
  ...
  const { to, firstName, courseName, paymentStatus, totalPaid, tuitionFee } = params
  ...
  const tuitionFormatted = tuitionFee !== null ? `₱${tuitionFee.toLocaleString('en-PH')}` : null
